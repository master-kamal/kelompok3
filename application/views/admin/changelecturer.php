                ...
                <div class="container-fluid">
					...
					<form action="" method="post">
						<input type="hidden" name="id" value="<?= $userLecturer['id']; ?>">
	                    <div class="form-group">
                            <label for="name">NIM</label>
						    <input type="text" name="nim" class="form-control" id="nim" value="<?=$userLecturer['nim']; ?>" readonly>
                        </div>
	                    <div class="form-group">
						    <label for="name">Name</label>
						    <input type="text" name="name" class="form-control" id="name" value="<?=$userLecturer['name']; ?>">
						    <?= form_error('name','<small class="text-danger ml-3">', '</small>'); ?>
						</div>
						<div class="form-group">
						    <label for="email">Email</label>
						    <input type="text" name="email" class="form-control" id="email" value="<?=$userLecturer['email']; ?>">
						    <?= form_error('email','<small class="text-danger ml-3">', '</small>'); ?>
						</div>
						<div class="form-group">
						    <select name="role_id" id="role_id" class="form-control">
							  <?php foreach ($userAllRole as $uA) : ?>
							  	<?php if( $uA['id'] == $userLecturer['role_id']) : ?>
